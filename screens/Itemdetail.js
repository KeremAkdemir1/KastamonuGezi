...
const Itemdetail = ({ route }) => {
    ...
    useEffect(() => {
        const unsubscribe = navigation.addListener('focus', () => {
            getFavorites()
            if (masterData === undefined && masterData[0] === null) {
                setDisplay('none')
            }
            else {
                setDisplay('flex')
    ...
    photos.forEach(element => {
        images.push("https://kastamonutravelguide.com.tr/" + element.filepath)
    })

    const imageViewer = [];
    photos.forEach(element => {
        imageViewer.push({
            url: "https://kastamonutravelguide.com.tr/" + element.filepath,
            props: {
            ...
            >
                ...
                {route.params?.youtube !== null ?
                    <View >
                        ...
                        />
                            <RenderHTML 
        contentWidth={Dimensions.get('screen').width} // Ekran genişliğine göre ayarlayın
        source={{ html: htmlContent }} 
      />
                    </View>
                    :
<View style={{padding:20,top:-60}}>
                    <RenderHTML 
                    contentWidth={Dimensions.get('screen').width} // Ekran genişliğine göre ayarlayın
                    source={{ html: htmlContent }} 
                  />
